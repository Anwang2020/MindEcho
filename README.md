...
| Text2SQL（智能体能力） | Agent 自动识别数据库问题，读取库表结构、生成只读 SQL 并将结果整理为表格 |
...
│   │   │       ├── process_word/
│   │   │       └── process_excel/
│   │   └── text2sql/            # Agent 内部使用：结构读取、SQL 生成与只读执行
...
Text2SQL 不需要单独打开页面、填写 SQL 或调用接口。它是 MindEcho 对话智能体的一部分：准备好数据库后，直接在桌面客户端或聊天连接里用自然语言提问即可。

第一次使用只需完成一次准备：

1. 找到要查询的本地 SQLite 数据库文件（通常以 `.db`、`.sqlite` 或 `.sqlite3` 结尾）。如果数据库由业务系统维护，请先让管理员提供**只读副本**，不要使用生产库的管理员账号。
2. 打开项目根目录的 `.env` 文件，在末尾添加一行；将示例中的路径换成你的数据库文件路径：

   ```dotenv
   TEXT2SQL_DATABASE_URL=sqlite:///D:/data/business.db
   ```

3. 启动 `python desktop.py`，然后像平时聊天一样提问。例如：
   - “这个月每个地区的销售额是多少？”
   - “销量最高的十个产品是什么？”
   - “把上周没有完成的订单列出来。”

智能体会自动判断该问题需要查询数据库，读取可用的表结构、生成查询并把结果整理为表格返回。你不需要知道表名或 SQL 语法。

为保护数据，Text2SQL 只执行一条只读查询，不会新增、修改或删除数据；默认最多显示 100 行。数据很多时，可以把问题说得更具体，例如“只看本月”“按地区汇总”“显示前十条”。如需修改行数，可在 `.env` 添加 `TEXT2SQL_MAX_ROWS=100`。

注意：Text2SQL 查询的是数据库文件，不是 Excel。Excel、Word、PDF 等文件应通过“上传文件”进入知识库后再提问；没有配置 `TEXT2SQL_DATABASE_URL` 时，智能体会提示尚未连接数据源。
