...
logger = logging.getLogger('book.tsr')
...
class TableStructureRecognizer(Recognizer):
    ...
    def __init__(self):
        try:
            super().__init__(self.labels, "tsr", "./model/rag/res/deepdoc")
        except Exception:
            super().__init__(
                self.labels,
                "tsr",
    ...
    def blockType(b):
        def _is_mostly_digits(text: str) -> bool:
            ...
            if not text_clean:
                return False
            # 匹配数字相关字符（0-9、小数点、千分位逗号、正负号）
            digit_match = re.findall(r'[0-9.,+-]', text_clean)
            # 计算数字占比并判断是否≥80%
            ratio = len(digit_match) / len(text_clean)
            return ratio >= 0.8
        ...
        for p, n in patt:
            if re.search(p, b["text"].strip()):
                return n
        if _is_mostly_digits(b["text"].strip()):
            return "Nr"
        stop_words = [
            '.', ',', '!', '?', ';', ':', '"', "'", '(', ')', '[', ']', '{', '}', '<', '>',
            '-', '_', '=', '+', '*', '/', '\\', '%', '$', '#', '@', '^', '&', '`', '~',
        ...
        tks = [w for w in jieba.lcut(b["text"]) if w not in stop_words]
