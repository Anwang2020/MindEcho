...
from apps.db.models import ChatSession, ChatInfo
            ...
            with self._get_session() as session:
                ...
                try:
                    if isinstance(record, ChatSession):
                        existing_session = session.get(ChatSession, record.id)
                        if existing_session:
                            existing_session.create_time = record.create_time
                            existing_session.llm_name = record.llm_name
                            existing_session.summary = record.summary
                            existing_session.chat_type = record.chat_type
                            existing_session.brief = record.brief
                            existing_session.create_by = record.create_by
                            logger.info(f"更新 ChatSession: id={existing_session.id}")
                        else:
                            session.add(record)
                    ...
                    session.rollback()
                    logger.error(f"插入 Chat 失败: brief={record.brief}, error={e}")
